...
| Same-origin Fastify runtime | CI smoke gate versioned; local HTTPS verification done; production topology ready | `verify:compose-runtime` will build/start the versioned Nginx + Fastify topology in CI and checks health, legacy redirects and response headers. `verify:same-origin-runtime` adds HTTPS health and provider redirect checks. Platform ingress, secrets and client registration remain release operations. |
| Production visual and accessibility gate | Done for public surfaces | CI builds the static export, runs committed axe/visual baselines at 1440/768/320, and checks dark reduced-motion plus keyboard skip navigation. Authenticated workspace remains pending trusted credential E2E. |
| Auth-transfer measurement | Done in application code; production Redis configuration pending platform rollout | Browser beacon correlates a tab-scoped anonymous intent and callback terminal state; Fastify rejects unknown fields, rate-limits to 30 events/IP/minute and logs `auth_transfer_viewed`. `REDIS_URL` makes the limit shared across API replicas; missing Redis is only an in-memory development fallback. Dashboard retention and alerts remain an observability operation. |
