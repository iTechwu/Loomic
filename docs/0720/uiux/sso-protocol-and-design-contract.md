...
| Same-origin Fastify runtime | Done locally; production topology ready | `verify:same-origin-runtime` checks HTTPS health and legacy redirects. `deploy/docker-compose.yml` versions the Web Nginx + private Fastify/worker topology; platform ingress, secrets and client registration remain release operations. |
...
| Auth-transfer measurement | Done in application code | Browser beacon correlates a tab-scoped anonymous intent and callback terminal state; Fastify rejects unknown fields, rate-limits to 30 events/IP/minute and logs `auth_transfer_viewed`. Dashboard retention and alerts remain an observability operation. |
