...
registerExecutor("image_generation", async (jobId, _rawPayload, ctx: ExecutorContext) => {
  ...
  try {
    ...
    if (!createdBy) throw new Error(`Job ${jobId} has no creator`);
    const assetRow = await ctx.dataRepository.createAsset({ bucket: "project-assets", byteSize: buffer.length, createdBy, etag: uploaded.etag, mimeType: generated.mimeType ?? "image/png", objectPath, ...(jobRow.project_id ? { projectId: jobRow.project_id } : {}), workspaceId });
